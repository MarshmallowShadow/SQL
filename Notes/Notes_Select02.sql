...
--묶은 row들 끼리 salary의 평균 구함
select job_id, avg(salary) from employees
group by job_id;

--오류: group by column명이 아닌 column입력하면 오류
...
--country_name 과 first_name 의 모든 조합 출력 (약 2675 데이터 출력)
...
--<left outer join>
--em에서 department_id가 null일 경우에도 (+)를 통해서 null있는 데이터 출력 가능
select first_name, em.department_id, 
       department_name, de.department_id
...
--<right outer join>
--left랑 right는 테이블의 위치 차이 (왼쪽테이블 기준으로 오른쪽에만 null이 없을 경우 출력)
select first_name, em.department_id, 
       department_name, de.department_id
...
--<full outer join>
--right outer join이랑 left outer join 포함 데이터 다 들어감
select first_name, em.department_id, 
       department_name, de.department_id
...
--<self join>
--같은 테이블인 employees(em)랑 employees(man)랑 비교해서 연결고리 있는 데이터 출력
--alias 이용 필수
