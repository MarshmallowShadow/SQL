...
select first_name || ' ' || last_name "이름",
	   hire_date
from employees
where first_name = 'John'; --String은 '='랑 '!='만 이용 가능

select first_name || ' ' || last_name "이름",
	   hire_date
from employees
where hire_date >= '07/01/01'; --겉으로만 String같이 보이지만 날짜 형식 (String이랑 다름)
...
select first_name || ' ' || last_name "이름",
	   salary
from employees
where salary >= 10000 and salary <= 14000; --자바에 &&랑 똑같음

select first_name || ' ' || last_name "이름"
from employees
where first_name = 'John' or first_name = 'Martha'; --자바에 ||랑 똑같음
...
from employees
where salary betweem 10000 and 14000; --salary가 10000이상('>=')  14000이하('<=") 일때만 조회

--in 연산자
select first_name || ' ' || last_name "이름", salary
from employees
where first_name in ('Martha', 'John'); --자바의 contains아님
--아래와 동일
select first_name || ' ' || last_name "이름"
from employees
...
select first_name || ' ' || last_name "이름", salary
from employees
where first_name like 'J%'; --맨 앞에 'J'들어간 이름 있는 데이터 조회 (대소문자 구분 주의)

select first_name || ' ' || last_name "이름", salary
from employees
where first_name like '%y'; --맨 뒤에 'y'들어간 이름 있는 데이터 조회

select first_name || ' ' || last_name "이름", salary
from employees
where first_name like '%h_';--'h'뒤에는 길이 상관없을(%), 앞에는 '_'가 하나만 있어서 'h'가 두번쨰로 마지막을 의미

select first_name
from employees
where first_name like '__a_';--'_'가 3개, 그리고 'a'가 있어서 길이는 4글자로 고정, 근데 세번쨰 자리에는 무조건 'a'가 들어가야 함

-----------------------------------------------------------------------------

--(null) 0이랑 다름 (자바의 null이랑 같은 개념)

select first_name, commission_pct
from employees
where commission_pct = null;--아무것도 안뜸, 틀린 방식 ('!='도 마찬가지)

select first_name, commission_pct
from employees
where commission_pct is null;--'=' 대신 'is'가 맞음

select first_name, manager_id
from employees
where manager_id is not null;--'!=' 대신 'is not'가 맞음
...
from employees
order by department_id; --기본이 오르는 순서

select first_name, salary
from employees
order by salary desc; --desc 내리는 순서

select department_id, salary, first_name
from employees
order by department_id asc, salary desc; --asc 오르는 순서, 먼저 입력한 column이 우선순위
...
select lpad(rpad(first_name, 10, '*'), 15, '*') from employees; --양쪽에 채울려면 이런 방식으로 하면 됨

select first_name, lpad(first_name, 3, '*') from employees; --길이가 더 짧으면 잘라버림
select first_name, rpad(first_name, 3, '*') from employees; --rpad도 오른쪽에서 자름
...
select trunc(3.1416, 3) from dual; --truncate(숫자, 소수자리) 내림
...
select first_name, to_char(salary, '99999') Salary from employees; --9의 길이만큼 문자로 표시 (적으면 공백으로 채워짐, salary가 더 길면 #####로 표시)

select first_name, to_char(salary, '099999') Salary from employees; --0과 9의 길이만큼 문자로 표시 (적으면 공백으로 채워짐)

select first_name, to_char(salary, '$99999') Salary from employees; --숫자 뒤에 '$' 붙임

select first_name, to_char(salary, '$09999.99') Salary from employees; --0과 9의 길이만큼 문자로 표시하고 뒤에 '$'붙임

select first_name, to_char(salary, '9999.99') Salary from employees; --'.'기준으로소수점 이하 표시

select first_name, to_char(salary, '99,999.99') Salary from employees; --해당 위치 사이에 ',' 표시

select first_name, to_char(salary, '$09,999.99') Salary from employees;

select first_name, to_char(salary, '000000') Salary from employees; --9대신 0으로 대체 가능

select first_name, to_char(salary, '333333') Salary from employees; --다른 숫자로 대체 하면은 오류 생김


--to_char(날짜, 문자형식)

select sysdate, to_char(sysdate, 'YYYY-MM-DD HH24:MI:SS') from dual; --날짜를 다른 형식으로 변경
...
select to_date('2022/05/04') from dual; --문자열을 날짜형식으로 변경 (날짜 함수에 기준 참고)
--반환 후에 날짜 관련 연산자 이용 가능


--to_number(문자)

select to_number('0099') from dual; --문자열을 숫자로 반환 (반환하면 숫자 관련 연산자 이용 가능)


--nvl(컬럼명, null일때값), nvl2(컬럼명, null아닐때값, null일때값)

select commission_pct, nvl(commission_pct, 0) from employees; --(null)을 전부 0으로 바꿈

select commission_pct, nvl2(commission_pct, 1, 0) from employees; --(null)이면 0으로 바꿈, null 아니면 전부 1로 바꿈
